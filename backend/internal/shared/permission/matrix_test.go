...
func TestAllowedDefaultsToDeny(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "owner can read finance summary", role: string(RoleOwner), permission: FinanceReadSummary, want: true},
		{name: "manager can read finance report", role: string(RoleManager), permission: FinanceReadReport, want: true},
		{name: "staff cannot read finance summary", role: string(RoleStaff), permission: FinanceReadSummary, want: false},
		{name: "staff cannot read finance expenses", role: string(RoleStaff), permission: FinanceReadExpense, want: false},
		{name: "cashier cannot read finance report", role: string(RoleCashier), permission: FinanceReadReport, want: false},
