          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <label className="space-y-1">
                <span className="text-sm font-medium text-[#6F6256]">Nomor pesanan</span>
                <Input
                  value={orderNumber}
              ...
              <label className="space-y-1">
                <span className="text-sm font-medium text-[#6F6256]">Nomor HP / WhatsApp</span>
                <Input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="08123456789" />
              </label>

              <Button className="w-full bg-[#251F1A] hover:bg-[#16110E]" type="submit" isLoading={trackingQuery.isFetching} disabled={trackingQuery.isFetching}>
                Lacak pesanan
              </Button>
            </form>
          </CardContent>
        ...
        <CardContent className="space-y-3">
          {data.items.map((item) => (
            <div key={`${item.productName}-${item.quantity}-${item.subtotal}`} className="flex items-start justify-between gap-4 text-sm">
              <div>
                <p className="font-semibold text-[#251F1A]">{item.productName}</p>
                <p className="mt-1 text-[#6F6256]">
                  {item.quantity} × {formatRupiah(item.unitPrice)}
                </p>
              </div>
              <p className="font-semibold text-[#251F1A]">{formatRupiah(item.subtotal)}</p>
            </div>
          ))}
        </CardContent>
      </Card>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-wide text-[#6F6256]">{label}</p>
      <p className="mt-1 text-[#6F6256]">{value}</p>
    </div>
  );
}
...
function PublicTimeline({ timeline }: { timeline: PublicTrackingTimelineItem[] }) {
  if (timeline.length === 0) {
    return <p className="text-sm text-[#6F6256]">Belum ada update pengiriman.</p>;
  }
    ...
    <div className="space-y-4">
      {timeline.map((item) => (
        <div key={`${item.status}-${item.createdAt}`} className="border-l-2 border-[#B96E45] pl-4">
          ...
          <p className="mt-1 text-xs text-[#6F6256]">{formatDateTime(item.createdAt)}</p>
